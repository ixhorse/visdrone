...
if __name__ == '__main__':
    ...
    for img_path in tqdm(train_list, ncols=80):
        ...
        label = utils.get_box_label(img_path)
        for box in label:
    ...
    plt.hist(origin_bboxsize, bins=10, density=True, label='origin', alpha=0.4)
    plt.hist(after_bboxsize, bins=10, density=True, label='after', alpha=0.4)
    plt.legend()
    plt.xlabel('size')
    plt.ylabel('count')
